...
const mergeClassicPositionInteractions = (
  ...
  const mergedByTxId = interactions.reduce(
    (acc, interaction) => {
      ...
      const usd: BalanceDelta = {
        balance: total.balance.mul(underlyingToNative), // TODO fix, needs share->underlying conversion
        delta: total.delta.mul(underlyingToNative), // TODO fix, needs share->underlying conversion
      };
      const txHash: string = interaction.createdWith.hash;
...
const classicPositionToInteractions = (
  ...
  const shareToken = toToken(position.vaultSharesToken);
  const underlyingToken = toToken(position.underlyingToken);
  const orderedRewardPoolTokenAddresses = sortEntitiesByOrderList(
    position.rewardPoolTokens,
    'address',
    position.rewardPoolTokensOrder
...
export async function getInvestorTimeline(
  ...
  const clmInteractions = clmPositions.flatMap(({ position, interactions }) =>
    clmPositionToInteractions(position.chain, position.clm, interactions)
  );
  const classicInteractions = classicPositions.flatMap(({ position, interactions }) =>
    classicPositionToInteractions(position.chain, position.classic, interactions)
  );
